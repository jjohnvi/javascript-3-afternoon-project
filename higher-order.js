...
//Code Here
let totalPopulation = populations.reduce(function(total, val, index, array) {
  return total + val;
}, 0);
...
//Code Here
let myStrongest = monstersInYourPocket.filter(function(val, index, array) {
  return val["CP"] > 200;
});
...
let orderTotals = orders.map(function(val) {
...
let bobsTotal = purchases.reduce(function(total, val) {
  if (val.owner === "Bob") {
    total += val.price;
  }
  return total;
